...
import {deleteUserByIdUsingPost1, listUserByPageUsingPost1} from "@/services/onlineJudge/adminBackend/userAdminController";
...
const UserAdminPage: React.FC = () =>
{
    ...
    // 当前用户点击的数据
    const [ currentRow, setCurrentRow ] = useState<API.UserAdminVO>({});
    const [ queryHandler, isLoading] = useAsyncHandler<{
        code?: number;
        data?: API.PageUserAdminVO_;
        message?: string;
    }>()
     ...
     * @param row
     */
    const handleDelete = async (row: API.UserAdminVO) =>
    {
        const hide = message.loading('正在删除');
        if (!row) return true;
        try
        {
            await deleteUserByIdUsingPost1({
                id: row.id as any,
            });
            hide();
